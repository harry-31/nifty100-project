...
from utils.db import get_sectors, get_peers, get_ratios
...
METRICS = [
    ...
    {"column": "return_on_capital_employed_pct",  "label": "ROCE %",        "higher_is_better": True},
    {"column": "net_profit_margin_pct",           "label": "Net Margin %", "higher_is_better": True},
    {"column": "operating_profit_margin_pct",     "label": "OPM %",         "higher_is_better": True},
    ...
    {"column": "dividend_yield_pct",               "label": "Dividend Yield %", "higher_is_better": True},
]
...
if not groups:
    st.warning("No peer groups found. Check that the `sectors` table has a `peer_group` column populated.")
    st.stop()
...
with col_b:
    benchmark_company = st.selectbox("Benchmark Company", group_df["company_name"].tolist())
...
if not available_metrics:
    ...
    st.caption("Values are normalised 0–100 within the peer group for comparability. Raw figures are in the table below.")
...
rename_map = {"company_name": "Company"} | {m["column"]: m["label"] for m in available_metrics}
